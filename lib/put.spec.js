...
describe('put', function () {
	...
	it('should throw when no resource is passed', function () {
		expect(_put2['default'].bind(null, '')).toThrow();
		expect(_put2['default'].bind(null, '')).toThrowError('Invalid config');
	});

	it('should throw when params are not passed to a parent resource', function () {
		expect(_put2['default'].bind(null, 'users.comments')).toThrow();
		expect(_put2['default'].bind(null, 'users.comments')).toThrowError('Invalid params: param is required for resource users');
	});
});